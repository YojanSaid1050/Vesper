...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Configura los canales de notificación de YouTube')
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'live') {
      ...
      updateGuildSection(interaction.guildId, 'youtube', { ...current, liveChannel: channel.id });
      return interaction.editReply({ content: `✅ Canal de directos configurado: <#${channel.id}>` });
    }

    if (subcommand === 'videos') {
      const channel = interaction.options.getChannel('canal');
      updateGuildSection(interaction.guildId, 'youtube', { ...current, videoChannel: channel.id });
      return interaction.editReply({ content: `✅ Canal de videos configurado: <#${channel.id}>` });
    }

    if (subcommand === 'shorts') {
      const channel = interaction.options.getChannel('canal');
      updateGuildSection(interaction.guildId, 'youtube', { ...current, shortChannel: channel.id });
    ...
    if (subcommand === 'clear') {
      ...
      let mensaje = '';
      switch (tipo) {
        case 'live': newConfig.liveChannel = null; mensaje = '✅ Canal de directos eliminado.'; break;
        case 'videos': newConfig.videoChannel = null; mensaje = '✅ Canal de videos eliminado.'; break;
        case 'shorts': newConfig.shortChannel = null; mensaje = '✅ Canal de shorts eliminado.'; break;
        case 'all': newConfig.liveChannel = null; newConfig.videoChannel = null; newConfig.shortChannel = null; mensaje = '✅ Todos los canales de notificación fueron eliminados.'; break;
      }
      updateGuildSection(interaction.guildId, 'youtube', newConfig);
