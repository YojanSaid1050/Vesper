class ErrorHandler {
  ...
  static logError(error, context, metadata = {}) {
    ...
    if (process.env.LOG_ERRORS === 'true') {
      ...
      const logDirectory = process.env.LOGS_PATH || path.join(process.cwd(), 'logs');
      const logPath = path.join(logDirectory, 'errors.ndjson');

      try {
        fs.mkdirSync(logDirectory, { recursive: true });

        if (fs.existsSync(logPath) && fs.statSync(logPath).size > 5 * 1024 * 1024) {
          const rotatedPath = path.join(logDirectory, `errors-${Date.now()}.ndjson`);
          fs.renameSync(logPath, rotatedPath);
        }

        fs.appendFileSync(logPath, `${JSON.stringify(logEntry)}\n`, 'utf8');
      } catch (logError) {
        console.error(`[ERROR][Logger] No se pudo persistir el error: ${logError.message}`);
      }
    }
  }
}
