...
class MonitorService {
  constructor(options) {
    ...
    this.errorCooldown = options.errorCooldown || Number(process.env.MONITOR_ERROR_COOLDOWN_MS) || 300000;
  }
  ...
  async run() {
    ...
    try {
      ...
      if (result && typeof result === 'object' && result.success === false) {
        const failure = new Error(result.error || result.reason || `${this.name} devolvió un resultado fallido`);
      ...
      if (this.consecutiveErrors >= this.maxConsecutiveErrors) {
        this.disabledUntil = Date.now() + this.errorCooldown;
        console.error(`⏸️ Monitor ${this.name} en pausa hasta ${new Date(this.disabledUntil).toISOString()} por ${this.consecutiveErrors} errores consecutivos`);
      }
